...
const jsonDataUrl2 = 'https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json'
    ...
    //Create base map variable to show
    let baseMaps = {
        Satelite: satelite,
        Grayscale: light,
        Outdoors: outdoors
...
d3.json(jsonDataUrl).then(data=>{
    ...
    d3.json(jsonDataUrl2).then(data=>{
      ...
      data.features.forEach(fl=>{
          //console.log(fl.geometry)
          boudLines.push(
              L.geoJSON([fl.geometry]) //Add popup when clicked w/ more info
          );
      })
      let boundLayer = L.layerGroup(boudLines);

      //Overlays for toggle on or off
      let overlayMaps = {
      ...
      //Pass map layers to layer control and the layer control to the map
